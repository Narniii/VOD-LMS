...
// const LOCALHOST = process.env.REACT_APP_LOCALHOST ? process.env.REACT_APP_LOCALHOST : false;
const LOCALHOST = false;
export function REACT_SITE_URL(path) {
    if (path.indexOf('http') !== -1)
        return path;
    if (path.startsWith('/'))
        path = path.substring(1);
    if (path.endsWith('/'))
        path = path.substring(0, path.length - 1);
    return LOCALHOST ? ('http://localhost:3000/' + path) : ('http://deadcat.ir:2525/' + path);
}
export function SITE_URL(path) {
    ...
    if (path.endsWith('/'))
        path = path.substring(0, path.length - 1);
    return LOCALHOST ? ('http://localhost:2424/' + path) : ('http://deadcat.ir:2424/' + path);
}
