...
export default function NewTest(props) {
    ...
    useEffect(() => {
        ...
        const localquiz = localStorage.getItem(`quiz${id}`)
        if (!localquiz) {
            localStorage.setItem(`quiz${id}`, parseFloat(score))
        }
        getUserQuiz()
        return () => {
    ...
    const handleShuffling = async (que) => {
        // console.log(que)
        if (que != undefined) {
            let questions = que
            let shuffledArray = await questions.sort((a, b) => 0.5 - Math.random());
        ...
        try {
            ...
            if (ress.data.message == 'Valid token') {
                if (ress.data.data) {
                    ...
                    let userCourseIdFromApi = await getAllUserCourses(quizz)
                    try {
                        apiCall.current = API.request(`/course/user/${userCourseIdFromApi}`, false, {}, globalUser.accessToken);
                        const rest = await apiCall.current.promise
                        for (var g = 0; g < rest.data.length; g++) {
                            if (globalUser.user_id == rest.data[g].user_id) {
                                // console.log(rest.data[g].quiz_score);
                                if (rest.data[g].quiz_score == 0)
                                    setQuizScoire(0)
                                quizFuckingScore = rest.data[g].quiz_score
                            }
                        }
                    } catch (err) {
                        setError(err)
                    ...
                    if (ress.data.data.length == 0) {
                        soalHa = await handleShuffling(que)
                        ...
                        if (quizFuckingScore != undefined) {
                            setQuizScoire(quizFuckingScore)
                            if (quizFuckingScore < 80) {
    ...
    */
    ...
    const getAllUserCourses = async (quiz) => {
        try {
                            ...
                            {showScore ?
                                ...
                                <div className={classes.questionsInputWrapper}>
                                    {quizScoire > 80 ? <>
                                        ...
                                        :
                                        <>
                                            {shuffledQuestions != undefined && shuffledQuestions.length != 0 ?
                                                <>
                                                    <Box sx={{ width: '100%' }}>
