...
export default function NewPost() {
    ...
    const submitCourse = (e) => {
        ...
        if (course.title == '' || course.title == undefined) {
            setError('عنوان خبر نمیتواند خالی باشد')
            return
        }
        if (course.short_description == '' || course.short_description == undefined) {
            setError('چکیده خبر نمیتواند خالی باشد')
            return
        }
        if (course.content == '' || course.content == undefined) {
            setError('درباره خبر نمیتواند خالی باشد')
            return
        }
        if (course.image == undefined) {
            setError('عکس خبر نمیتواند خالی باشد')
            return
        }
        if (tags.length == 0) {
            setError('تگ های خبر نمیتواند خالی باشد')
            return
        }
        ...
        fetch('https://api.aqua.ir:8283/post/create/', { // Your POST endpoint
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${globalUser.accessToken}`
            },
    ...
    const catChange = (e) => {
        ...
        if (e.target.value != -1 && selectedCats != e.target.value) {
            ...
            let newCat = {
                id: e.target.value,
                catName: e.nativeEvent.target[index].text
            }
    ...
    return (
        <>
            ...
            <Navbar children={<UserDashboard currentTab={"post"} firstChildSelected={true} />} />
            <section
                    ...
                    <div className={classes.whiteBox}>
                        <div className="row">
                            <div className="col-sm-3 no-mobile">
                                <UserDashboard currentTab={"post"} firstChildSelected={true} />
                            </div>
                            ...
                            <div className="col-sm-9" >
                                <div className={classes.khakibox}>
                                    {loading ?
                                        ...
                                        :
                                        <>
                                            <h3 className={classes.createHeader} >ساخت خبر</h3>
                                            <div className={classes.inputsWrapper}>
                                                <div className="create-course-inputs">
                                                    ...
                                                    <div className={classes.inputsHeader}> متن خبر</div>
                                                    ...
                                                    <div style={{ margin: '10px 0px' }}>
                                                        <ReactFileReader multipleFiles={false} style={{ display: 'inline-block !important' }} handleFiles={handleFiles}>
                                                            <span className={classes.inputsHeader} style={{ display: 'inline-block !important' }}>عکس خبر:</span>
                                                    ...
                                                    <div className={classes.submitButtonWrapper}>
                                                        ...
                                                        <button className={classes.whiteBtn} onClick={() => { history.push('/welcome') }}>انصراف</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </>
