...
export default function AvailableQuizes() {
  ...
  const [selectedUserStatus, setSelectedUserStatus] = useState(undefined)
  ...
  const statusChange = async (e) => {
    ...
    } else setSelectedUserStatus(0)
  }
  const submit = async () => {
    try {
      apiCall.current = API.request('/user/profile/edit/status/', true, {
  ...
  return (
    <>
      <Helmet>
        <title>تغییر وضعیت کاربر</title>
      </Helmet>
      <Navbar children={<UserDashboard currentTab={"edituser"} secondChildSelected={true} />} />
          ...
          <div className={classes.whiteBox}>
            <div className="row">
              <div className="col-sm-3 no-mobile">
                <UserDashboard currentTab={"edituser"} secondChildSelected={true} />
              </div>
                ...
                <div className={classes.khakibox}>
                  {loading ? <div style={{
                    ...
                    <div className={classes.coursesInProgressWrapper}>
                      <h4 className="text-center" style={{ margin: '20px 0px', color: '#64c5ba', fontWeight: 'bold !important' }}>تغییر وضعیت کاربر</h4>
