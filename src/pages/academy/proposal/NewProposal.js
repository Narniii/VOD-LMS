...
export default function NewProposal() {
    ...
    const submitCourse = (e) => {
        ...
        if (course.title == '' || course.title == undefined) {
            setError('عنوان پروپوزال نمیتواند خالی باشد')
            return
        }
        if (course.short_description == '' || course.short_description == undefined) {
            setError('چکیده پروپوزال نمیتواند خالی باشد')
            return
        }
        if (course.content == '' || course.content == undefined) {
            setError('درباره پروپوزال نمیتواند خالی باشد')
            return
        }
        if (course.image == undefined) {
            setError('عکس پروپوزال نمیتواند خالی باشد')
            return
        }
        if (tags.length == 0) {
            setError('تگ های پروپوزال نمیتواند خالی باشد')
            return
        }
        setLoading(true)
        ...
        fetch('https://api.aqua.ir:8283/proposal/create/', { // Your POST endpoint
            ...
            success => {
                ...
                if (success.message == 'Valid token') {
                    setLoading(false)
                    setSuccessMessage('پروپوزال ساخته شد')
                    setError(undefined)
                }
                                    ...
                                    {loading ?
                                        ...
                                        <>
                                            {globalUser.user_group == "admin" ?
                                                <>
                                                    <h3 className={classes.createHeader} >ساخت پروپوزال</h3>
                                                    <div className={classes.inputsWrapper}>
                                                        <div className="create-course-inputs">
                                                            ...
                                                            <div className={classes.inputsHeader}> متن پروپوزال</div>
                                                            ...
                                                            <div style={{ margin: '10px 0px' }}>
                                                                <ReactFileReader multipleFiles={false} style={{ display: 'inline-block !important' }} handleFiles={handleFiles}>
                                                                    <span className={classes.inputsHeader} style={{ display: 'inline-block !important' }}>عکس پروپوزال:</span>
                                                ...
                                                <h1 style={{ color: '#64c5ba' }}>شما دسترسی به این صفحه ندارید</h1>}
                                        </>
                                    }
                                </div>
                            </div>
